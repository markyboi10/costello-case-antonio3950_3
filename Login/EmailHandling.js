  ...
  if (jwtData && jwtData.email_verified === true) {
    ...
    fetch(`/initUser?email=${jwtData.email}`)
      ...
      .then((data) => {
        ...
        localStorage.setItem("loginEmail", jwtData.email);
        // Redirect to a different user profile home page
  ...
  fetch(
    ...
    .then((data) => {
      ...
      if (data.error) {
         ...
         */
        localStorage.setItem("userData", JSON.stringify(data));
        const loginEmail = document.getElementById("emailAddress").value;
...
function decodeSignUpResponse() {
  ...
  fetch(
    `/initNewUser?formEmail=${document.getElementById("signupEmail").value}`
  )
    .then((response) => response.json())
    .then((data) => {
